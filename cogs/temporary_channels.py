...
class TemporaryChannels(commands.Cog):
    def __init__(self, bot, log):
        ...
        # maps category to number_manager
        self._category_room_number_manager = {}
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        with self._temporary_channels_cache.lock:
            ...
            if before.channel and before.channel.id in self._temporary_channels_cache.get_list():
                try:
                    ...
                    if len(members) == 0:
                        ...
                        if before.channel.category.id in self._category_room_number_manager:
                            self._category_room_number_manager[before.channel.category.id].free(before.channel.id)
                except Exception as e:
                    self.log.warning('Can\'t delete temporary channel \'{}\': {}'.format(before.channel.name, str(e)))

            # ignore events generated by non generator channels
            ...
            # create RoomNumberManager for the category if doesn't exists
            if after.channel.category.id not in self._category_room_number_manager:
                    self._category_room_number_manager[after.channel.category.id] = self.RoomNumberManager()
            room_number_manager = self._category_room_number_manager[after.channel.category.id]

            # create a channel
            guild = after.channel.guild
            new_channel = None
            with room_number_manager.lock:
                room_number = room_number_manager.next_room_number()
                ...
                try:
                    ...
                    self.log.warning('Can\'t create temporary channel \'{}\': {}'.format(new_channel_name, str(e)))
                    return
    ...
    class RoomNumberManager:
        ...
        def next_room_number(self, commit=False):
            with self.lock:
                if len(self._freed_room_numbers) == 0:
                    res = self._highest_room_number + 1
                    if commit:
                        self._highest_room_number += 1
                    return res
                else:
                    least = self._freed_room_numbers[0]
                    if commit:
                        self._freed_room_numbers.pop(0)
                    return least

        def assign(self, channel_id):
            self._channel_room_number[channel_id] = self.next_room_number(commit=True)

        def free(self, channel_id):
            with self.lock:
